...
export function LineUserRoute({ children }: { children: ReactNode }) {
  const { isAuthenticated, isLoading, user } = useAuth();
  const location = useLocation();

  if (isLoading) return <AuthLoading />;

  if (!isAuthenticated || user?.authProvider !== 'LINE' || user.role !== 'USER') {
...
export function AdminRoute({ children }: { children: ReactNode }) {
  const { isAuthenticated, isLoading, user } = useAuth();
  if (isLoading) return <AuthLoading />;
  if (!isAuthenticated || user?.authProvider !== 'LOCAL_ADMIN' || user.role !== 'ADMIN') {
